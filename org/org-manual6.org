...
12.11 Markdown导出
Md导出后端为org mode buffer生成Markdown语法.
它是建立在HTML后端:任何构造没有被Markdown语法支持将被html后端约束和转化.
Markdown导出命令
C-c C-e m m
          导出作为一个text文件被写在Markdown语法里.作为一个org文件myfile.org,造成文件将是myfile.md.这文件将被覆盖外部警告.
C-c C-e m M
          导出一个临时缓存,不创建文件.b
C-c C-e m o
          导出一个带有Markdown语法的text文件,然后打开它.
标题和分段结构
Markdown导出能生成包括atx和setext类型作为标题,例如,org-md-headline-style.前者引入了两个层次的硬限制,而后者推到六
该限制下的标题列为列表.您也可以设置软限制之前.
12.12 开放文档格式 Text导出
Org模式支持OpenDocument text格式.文件创造被这导出者用penDocument-v1.2 specification26 和兼容 LibreOffice 3.4.
12.12．1 Pre-requisites 为ODT导出
ODT导出者依赖在zip项目去创造最终输出.检查这项目在进行之前.
12.12．2 ODT导出命令
C-c C-e o o
导出作为penDocument text文件.
如果org-odt-preferred-output-format被规定,自动转换导出文件到该格式.
对于一个org文件myfile.org,这ODT文件将是myfile.odt.该文件将被覆盖没有警告.如果有一个活跃区域,只有该区域导出.如果选定的区域是一个单一的树,树头将成为文档标题.如果树头项,或继承,一个export_file_名称属性,该名称将用于导出.
C-c C-e o O导出作为一个penDocument text文件和打开结果文件.
